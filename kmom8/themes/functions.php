...
function get_debug() {
  ...
  if(isset($ly->config['debug']['lycka']) && $ly->config['debug']['lycka']) {
    $html .= "<hr><h3>Debuginformation</h3><p>The content of CLycka:</p><pre>" . htmlent(print_r($ly, true)) . "</pre>";
  }    
  if(isset($ly->config['debug']['session']) && $ly->config['debug']['session']) {
    $html .= "<hr><h3>SESSION</h3><p>The content of CLycka->session:</p><pre>" . htmlent(print_r($ly->session, true)) . "</pre>";
...
function get_messages_from_session() {
  $messages = CLycka::Instance()->session->GetMessages();
  $html = null;
  if(!empty($messages)) {
...
function get_gravatar($size=null) {
  return 'http://www.gravatar.com/avatar/' . md5(strtolower(trim(CLycka::Instance()->user['email']))) . '.jpg?r=pg&amp;d=wavatar&amp;' . ($size ? "s=$size" : null);
}
...
function base_url($url=null) {
  return CLycka::Instance()->request->base_url . trim($url, '/');
}
...
function create_url($urlOrController=null, $method=null, $arguments=null) {
  return CLycka::Instance()->CreateUrl($urlOrController, $method, $arguments);
}
...
function theme_url($url) {
  return create_url(CLycka::Instance()->themeUrl . "/{$url}");
}
...
function theme_parent_url($url) {
  return create_url(CLycka::Instance()->themeParentUrl . "/{$url}");
}
...
function current_url() {
  return CLycka::Instance()->request->current_url;
}
...
function render_views($region='default') {
  return CLycka::Instance()->views->Render($region);
}
...
function region_has_content($region='default' /*...*/) {
  return CLycka::Instance()->views->RegionHasView(func_get_args());
}
